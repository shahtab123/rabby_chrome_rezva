...
const SelectToAddress = () => {
  ...
  const handleGotoSend = useCallback(
    (address: string, type?: string, extras?: { token?: string }) => {
      if (nftItem) {
        handleGotoSendNFT(address);
      } else {
        handleGotoSendToken(history, address, type, extras);
      }
    },
  ...
  const handleConfirmAccount = useCallback(
    (account?: { address: string; type?: string; token?: string } | null) => {
      if (!account) return;
      const { address, type, token } = account;
      ...
      handleGotoSend(address, type, token ? { token } : undefined);
    },
  ...
  const handleSelectAccount = useCallback(
    (account: { address: string; type?: string; token?: string }) => {
